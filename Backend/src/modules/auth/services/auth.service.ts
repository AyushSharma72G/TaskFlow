...
export class AuthService {
	...
	async getCurrentUserRole(userId: string, projectId: string): Promise<{
		projectId: string;
		role: string | null;
	}> {
		const role = await this.authRepository.findUserRoleInProject(userId, projectId);
		return {
			projectId,
			role,
		};
	...
	private signAccessToken(userId: string, email: string): string {
		const expiresIn = config.ACCESS_TOKEN_EXPIRES_IN as SignOptions['expiresIn'];
		return sign(
			{ sub: userId, email },
			config.JWT_SECRET,
			{
	...
	private signRefreshToken(userId: string, email: string): string {
		const expiresIn = config.REFRESH_TOKEN_EXPIRES_IN as SignOptions['expiresIn'];
		return sign(
			{ sub: userId, email },
	...
	private verifyRefreshToken(refreshToken: string): { sub: string; email: string } {
		try {
			const payload = verify(refreshToken, config.REFRESH_TOKEN_SECRET) as {
				sub?: string;
				email?: string;
			};
	...
	private async setRefreshToken(userId: string, refreshToken: string): Promise<void> {
		...
		await this.authRepository.updateRefreshTokenHash(userId, hashedRefreshToken);
	}
}
