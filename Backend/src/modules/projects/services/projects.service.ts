...
export class ProjectsService {
  ...
  async createProject(
    ...
    return this.projectsRepository.createProjectAndAddAdminMember({
      userId,
  ...
  async getProjectDetails(
    ...
    const membership = await this.projectsRepository.findProjectDetailsForMember(
      userId,
      projectId,
    );
  ...
  async updateProject(
    ...
    const hasAnyField =
      dto.title !== undefined || dto.description !== undefined || dto.dueDate !== undefined;
    if (!hasAnyField) {
  ...
  // DELETE /projects/:id
  async deleteProject(userId: string, projectId: string): Promise<{ success: true }> {
    const role = await this.projectsRepository.findUserRoleInProject(
      userId,
      projectId,
    );
